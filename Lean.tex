...
\CreateMacro{apply}
\CreateMacro[normnum]{norm\_num}
\CreateMacro[ccases]{cases}
\CreateMacro[rrepeat]{repeat}
...
\LeanCommand[sshow]{show}

\begin{itemize}
    \item \texttt{\sshow $g$} selects which of the goals to prove and unifies it with $g$.
\end{itemize}

\LeanCommand[renamei]{rename\_i} ???????? rename inaccessible variable (one with dagger)
...
\LeanCommand[nthrw]{nth\_rw}

\begin{itemize}
    \item \texttt{\nthrw $i$ $j$ [$h$]} rewrites $i$-th to $j$-th occurances of hypothesis $h$.
\end{itemize}
...
\LeanCommand[unfoldlet]{unfold\_let}
...
\begin{itemize}
    ...
    \item \texttt{example :$\;\exists x : \R, 41 < x \wedge x < 43 \; := \; $⟨$42$, by \normnum⟩}.
\end{itemize}

\LeanCommand[ddelta]{delta}

\LeanCommand{decide} ?????????? brute force trial for finite types

\LeanCommand[fincases]{fin\_cases} ????????? consider finite number of cases
...
\begin{itemize}
    \item Tactic combinator: \texttt{<;>} applies the subsequent tactic to all subgoals generated by \ccases.
    For example, \texttt{\ccases $h$; \rrepeat \assumption} tries to close all cases by using existing hypotheses. This can be abbreviated as \texttt{\ccases $h$ <;> \assumption}.
\end{itemize}

\subsection*{Tactics}

\LeanCommand[bycases]{by\_cases}

\begin{itemize}
    \item \texttt{\bycases $h : P$} splits the goal into two cases, assuming $h : P$ in the first branch and $h : \neg P$ in the second.
\end{itemize}
...
\LeanCommand[casesp]{cases'}

\begin{itemize}
    \item \texttt{\casesp $h$ with $hp$ $hq$} breaks up the hypothesis
    \begin{itemize}
    ...
    \end{itemize}
    \item \texttt{\casesp $h$ with $z$ $hf$} replaces hypothesis $h: \exists x : X, P x$ with $hf: P z$ and introduces a new hypothesis $z : X$.
    \item \texttt{\casesp $h$ $z$ with $x$ $hx$} uses hypothesis $h: \forall b : Z, \exists a : X, f a = b$ to create two new hypotheses: $x : X$ and $hx : f x = z$.
    \item If $n:\N$ then \texttt{\casesp $n$ with $m$} gives two goals: one where $n$ is replaced by $0$ and another where it is replaced by \texttt{succ $m$}.
\end{itemize}

\CreateTitle[ccases]{cases}

\begin{itemize}
    \item \ccases is the structured version of \casesp that uses \texttt{\ccases $h$ with} followed by \texttt{| constructor\_name => tactic} branches.
    \item \texttt{\ccases $h$ with | inl $hp$ => ... | inr $hq$ => ...} on $h : P \vee Q$ creates two goals:
    \begin{itemize}
        \item \texttt{| inl $hp$ =>} with hypothesis $hp : P$,
        \item \texttt{| inr $hq$ =>} with hypothesis $hq : Q$.
    \end{itemize}
    \item \texttt{\ccases $n$ with | zero => ... | succ $m$ => ...} on $n : \N$ creates two goals:
    \begin{itemize}
        \item \texttt{| zero =>} where $n$ is replaced by $0$,
        \item \texttt{| succ $m$ =>} where $n$ is replaced by \texttt{succ $m$}.
    \end{itemize}
    \item Unlike \casesp, the named branches can appear in any order, making proofs more robust to upstream changes.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{\rintro $hP$ ⟨$hQ$, $hR$⟩} is equivalent to \texttt{\intro $hP$} followed by \texttt{\intro $h$} and \texttt{\casesp $h$ with $hQ$ $hR$}.
...
\LeanCommand[intervalcases]{interval\_cases}

\begin{itemize}
    \item \texttt{\intervalcases $n$} searches for upper and lower bounds on a variable $n$, and if bounds are found, splits into separate cases for each possible value of $n$.
\end{itemize}
...
\CreateTitle[normnum]{norm\_num}

\begin{itemize}
    \item \normnum normalizes the numerical expression in the goal.
\end{itemize}
...
\LeanCommand[substvars]{subst\_vars}

\begin{itemize}
    \item \substvars performs all possible substitutions with \subst.
\end{itemize}
...
\LeanCommand[cleanvalue]{clean\_value} ??????????
...
\LeanCommand[ddef]{def}

\begin{itemize}
    \item \texttt{\ddef $f : \N \to \R$ := \fun $n \mapsto n^2 + 3$} defines a function.
\end{itemize}

\subsection*{Tactics}

\LeanCommand[applyfun]{apply\_fun}

\begin{itemize}
    \item \texttt{\applyfun $f$} turns goal $\vdash x = y$ into $\vdash f(x) = f(y)$.
    \item \texttt{\applyfun $f$ at $h$} turns hypothesis $h : x = y$ into $h : f(x) = f(y)$.
\end{itemize}

\LeanCommand[cchoose]{choose}

\begin{itemize}
    \item \texttt{\cchoose $g$ $hg$ using $sur$} creates a new hypothesis $hg : \forall (y : Y), f(g \ y) = y$ which asserts that $g : Y \to X$ is a one-sided inverse of function $f : X \to Y$ whose surjectivity is guaranteed by hypothesis \texttt{$sur$ : Function.Surjective $f$}.
\end{itemize}
...
\LeanCommand[llet]{let}

\LeanCommand[sset]{set}

\begin{itemize}
    \item \texttt{\sset $a$ := $t$ with $h$} is a variant of \texttt{\llet $a$ := $t$}. It adds the hypothesis \texttt{$h : a = t$} to the local context and replaces $t$ with $a$ everywhere it can.
\end{itemize}
...
\begin{itemize}
    \item \texttt{\ccases <;> \assumption} splits the proof into cases and uses \assumption in each case.
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{%
        ...
        let ⟨$x, hx$⟩ $:= h \; 2$ \\
        ⟨$x, hx \blacktriangleright$ by \normnum⟩%
    }
\end{itemize}

\paragraph{\textless\textbar}

\LeanCommand[ttry]{try}
...
\CreateTitle[rrepeat]{repeat}

\begin{itemize}
    \item \texttt{\rrepeat $t$} keeps applying tactic $t$ until it fails.
\end{itemize}

\LeanCommand[repeatp]{repeat'}
...
\LeanCommand[ringnf]{ring\_nf}

\begin{itemize}
    \item \ringnf rewrites the ring expression in the goal into a normal form.
\end{itemize}

\LeanCommand[noncommring]{noncomm\_ring}
...
\LeanCommand[linearcombination]{linear\_combination}

\LeanCommand{polyrith}

\begin{itemize}
    \item \polyrith finds the right coefficients for \linearcombination.
\end{itemize}
...
\LeanCommand[fieldsimp]{field\_simp}
...
\begin{itemize}
    ...
    \item $\vdash a \vee b$ -- use \lleft or \rright
